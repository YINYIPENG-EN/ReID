...
def detect(cfg,
    ...
    ############# 行人检测模型初始化 #############
    model = Darknet(cfg, img_size)     # 行人检测模型
    ...
    # Set Dataloader
    vid_path, vid_writer = None, None   # 视频路径，识别保存
    if opt.webcam:   # 网络视频
        save_images = False
        dataloader = LoadWebcam(img_size=img_size, half=opt.half)
    else:
        dataloader = LoadImages(images, img_size=img_size, half=opt.half)  # 此处的images即为sample中的
